import { C as b, c as f } from "../../chunks/runtime.js";
...
class L extends g {
  ...
  async keyDownHandler(e) {
    const { hasSubmenu: n, href: c, layout: h, open: a, submenuItems: d } = this, o = e.key, s = e.target === this.dropdownActionEl.value;
    if (!e.defaultPrevented) {
      if (o === " " || o === "Enter")
        n && (!c || c && s) && (this.open = !a), !(c && s) && o !== "Enter" && this.selectMenuItem(e), (o === " " || c && s) && e.preventDefault();
      else if (o === "Escape") {
        if (a) {
          this.open = !1;
          return;
  ...
  // #region Rendering
  renderIconStart() {
    return l(t.iconStart, i`<calcite-icon class=${`${t.icon} ${t.iconStart}`} .flipRtl=${this.iconFlipRtl === "start" || this.iconFlipRtl === "both"} .icon=${this.iconStart} scale=s></calcite-icon>`);
  }
  renderIconEnd() {
    return l(t.iconEnd, i`<calcite-icon class=${`${t.icon} ${t.iconEnd}`} .flipRtl=${this.iconFlipRtl === "end" || this.iconFlipRtl === "both"} .icon=${this.iconEnd} scale=s></calcite-icon>`);
  }
  renderBreadcrumbIcon(e) {
    return l(t.iconBreadcrumb, i`<calcite-icon class=${`${t.icon} ${t.iconBreadcrumb}`} .icon=${e === "rtl" ? "chevron-left" : "chevron-right"} scale=s></calcite-icon>`);
  }
  renderDropdownIcon(e) {
    const n = e === "rtl" ? "chevron-left" : "chevron-right";
    return l(t.iconDropdown, i`<calcite-icon class=${`${t.icon} ${t.iconDropdown}`} .icon=${this.topLevelMenuLayout === "vertical" || this.isTopLevelItem ? this.open ? "chevron-up" : "chevron-down" : n} scale=s></calcite-icon>`);
  }
  renderDropdownAction(e) {
    const n = e === "rtl" ? "chevron-left" : "chevron-right";
    return l(t.dropdownAction, i`<calcite-action class=${r(t.dropdownAction)} .icon=${this.topLevelMenuLayout === "vertical" || this.isTopLevelItem ? this.open ? "chevron-up" : "chevron-down" : n} @click=${this.clickHandler} @keydown=${this.keyDownHandler} .text=${this.messages.open} ${p(this.dropdownActionEl)}></calcite-action>`);
  }
  renderSubmenuItems(e) {
    return i`<calcite-menu class=${r({
  ...
  renderHrefIcon(e) {
    return l(t.hoverHrefIcon, i`<calcite-icon class=${r(t.hoverHrefIcon)} .icon=${e === "rtl" ? "arrow-left" : "arrow-right"} scale=s></calcite-icon>`);
  }
  renderItemContent(e) {
    const n = this.href && (this.topLevelMenuLayout === "vertical" || !this.isTopLevelItem);
    return i`${this.iconStart && this.renderIconStart() || ""}<div class=${r(t.textContainer)}><span>${this.text}</span></div>${n && this.renderHrefIcon(e) || ""}${this.iconEnd && this.renderIconEnd() || ""}${this.breadcrumb ? this.renderBreadcrumbIcon(e) : null}${!this.href && this.hasSubmenu ? this.renderDropdownIcon(e) : null}`;
  }
  render() {
    const e = w(this.el);
    return i`<li class=${r({
