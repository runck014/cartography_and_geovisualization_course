...
import { createRef as f, ref as i } from "lit-html/directives/ref.js";
import { literal as l, html as k } from "lit-html/static.js";
import { LitElement as m, stringOrBoolean as x, safeClassMap as a, nothing as e } from "@arcgis/lumina";
...
import { g as d } from "../../chunks/component.js";
...
class H extends m {
  ...
  // #region Rendering
  render() {
    const t = this.href ? "a" : "button", r = this.href ? l`a` : l`button`, u = this.loading ? o`<div class=${a(n.buttonLoader)}><calcite-loader class=${a(this.loading ? n.loadingIn : n.loadingOut)} inline .label=${this.messages.loading} .scale=${this.scale === "l" ? "m" : "s"}></calcite-loader></div>` : null, c = !this.iconStart && !this.iconEnd, b = o`<calcite-icon class=${a({ [n.icon]: !0, [n.iconStart]: !0 })} .flipRtl=${this.iconFlipRtl === "start" || this.iconFlipRtl === "both"} .icon=${this.iconStart} .scale=${d(this.scale)}></calcite-icon>`, p = o`<calcite-icon class=${a({ [n.icon]: !0, [n.iconEnd]: !0 })} .flipRtl=${this.iconFlipRtl === "end" || this.iconFlipRtl === "both"} .icon=${this.iconEnd} .scale=${d(this.scale)}></calcite-icon>`, h = o`<span class=${a(n.content)} ${i(this.contentEl)}><slot></slot></span>`;
    return C({ disabled: this.disabled, children: k`<${r} .ariaBusy=${this.loading} .ariaExpanded=${this.el.ariaExpanded ? this.el.ariaExpanded : null} .ariaLabel=${this.loading ? this.messages.loading : O(this)} aria-live=polite class=${a({
      [n.buttonPadding]: c,
      [n.buttonPaddingShrunk]: !c,
      [n.contentSlotted]: this.hasContent,
      [n.iconStartEmpty]: !this.iconStart,
      [n.iconEndEmpty]: !this.iconEnd
    })} .disabled=${t === "button" ? this.disabled : null} download=${(t === "a" ? this.download === !0 || this.download === "" ? "" : this.download || null : null) ?? e} href=${(t === "a" && this.href) ?? e} name=${(t === "button" && this.name) ?? e} @click=${this.handleClick} rel=${(t === "a" && this.rel) ?? e} tabindex=${(this.disabled ? -1 : null) ?? e} target=${(t === "a" && this.target) ?? e} title=${this.tooltipText ?? e} type=${(t === "button" ? this.type : null) ?? e} ${i(this.setChildEl)}>${u}${this.iconStart ? b : null}${this.hasContent ? h : null}${this.iconEnd ? p : null}</${r}>` });
  }
}
g("calcite-button", H);
export {
