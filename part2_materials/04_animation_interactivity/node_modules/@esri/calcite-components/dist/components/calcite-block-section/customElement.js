...
import { LitElement as p, createEvent as u, safeClassMap as c, nothing as b } from "@arcgis/lumina";
import { h as x } from "../../chunks/dom.js";
...
class S extends p {
  ...
  renderStatusIcon() {
    ...
    return o ? r`<calcite-icon class=${c(i)} .icon=${o} scale=s></calcite-icon>` : null;
  }
  renderIcon(e) {
    const { iconFlipRtl: o } = this;
    if (e === void 0)
      return null;
    const i = o === "both" || o === "start", a = o === "both" || o === "end", n = e === this.iconStart;
    return v(n ? this.iconStart : this.iconEnd, r`<calcite-icon class=${c(n ? t.iconStart : t.iconEnd)} .flipRtl=${n ? i : a} .icon=${n ? this.iconStart : this.iconEnd} scale=s></calcite-icon>`);
  }
  render() {
    const { messages: e, open: o, text: i, toggleDisplay: a } = this, n = o ? s.menuOpen : s.menuClosed, d = o ? e.collapse : e.expand, g = a === "switch" ? r`<div class=${c({
      [t.toggleContainer]: !0
    })}><div aria-controls=${l.content} .ariaExpanded=${o} class=${c({
      [t.toggle]: !0,
      [t.toggleSwitch]: !0
    })} id=${l.toggle} @click=${this.toggleSection} @keydown=${this.handleHeaderKeyDown} role=button tabindex=0 title=${d ?? b}>${this.renderIcon(this.iconStart)}<div class=${c(t.toggleSwitchContent)}><span class=${c(t.toggleSwitchText)}>${i}</span></div>${this.renderIcon(this.iconEnd)}${this.renderStatusIcon()}<calcite-switch .checked=${o} class=${c(t.switch)} inert .label=${d} scale=s></calcite-switch></div></div>` : r`<div class=${c({
      [t.toggleContainer]: !0
    ...
    return r`${g}<section aria-labelledby=${l.toggle} class=${c(t.content)} .hidden=${!o} id=${l.content}><slot></slot></section>`;
  }
}
h("calcite-block-section", S);
export {
  S as BlockSection
};
