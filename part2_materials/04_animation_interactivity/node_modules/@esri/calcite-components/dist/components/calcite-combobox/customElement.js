...
class Ae extends Y {
  ...
  constructor() {
    super(), this.filterItems = (() => {
      ...
      return X((t, i = !1, s = !0) => {
        const l = ee([...this.data, ...this.groupData], t, this.effectiveFilterProps), a = this.getItemsAndGroups(), o = t === "";
        a.forEach((n) => {
          if (o) {
            n.itemHidden = !1;
            return;
        ...
        }), this.filterTextMatchPattern = this.filterText && new RegExp(`(${J(this.filterText)})`, "i"), this.filteredItems = this.getFilteredItems(), this.filteredItems.forEach((n) => {
    ...
    }, this.groupItems = [], this.guid = pe(), this.ignoreSelectedEventsFlag = !1, this.inputHeight = 0, this.internalValueChangeFlag = !1, this.items = [], this.mutationObserver = E("mutation", () => this.updateItems()), this.onLabelClick = () => {
      this.setFocus();
    }, this.transitionProp = "opacity", this.placement = ie, this.resizeObserver = E("resize", () => {
      this.setMaxScrollerHeight(), this.refreshSelectionDisplay();
    }), this._selectedItems = [], this.textInput = Q(), this._value = null, this.activeChipIndex = -1, this.activeDescendant = "", this.activeItemIndex = -1, this.compactSelectionDisplay = !1, this.selectedHiddenChipsCount = 0, this.selectedVisibleChipsCount = 0, this.clearDisabled = !1, this.disabled = !1, this.filteredItems = [], this.maxItems = 0, this.messages = we(), this.open = !1, this.overlayPositioning = "absolute", this.placeholderIconFlipRtl = !1, this.readOnly = !1, this.required = !1, this.scale = "m", this.selectionDisplay = "all", this.selectionMode = "multiple", this.status = "idle", this.validity = {
      valid: !1,
      badInput: !1,
      customError: !1,
      patternMismatch: !1,
  ...
  connectedCallback() {
    super.connectedCallback(), fe(this), ne(this), this.internalValueChangeFlag = !0, this.value = this.getValue(), this.internalValueChangeFlag = !1, this.mutationObserver?.observe(this.el, { childList: !0, subtree: !0 }), this.setFilteredPlacements(), C(this);
  }
  async load() {
  }
  willUpdate(e) {
  ...
  updated() {
    this.el.offsetHeight !== this.inputHeight && (this.reposition(!0), this.inputHeight = this.el.offsetHeight), ue(this), this.refreshSelectionDisplay();
  }
  loaded() {
    ce(this, this.getValue()), C(this), this.updateItems(), this.filterItems(this.filterText, !1, !1);
  }
  ...
  openHandler() {
    ve(this), !this.disabled && this.setMaxScrollerHeight();
  }
  ...
  setFilteredPlacements() {
    ...
    this.filteredFlipPlacements = t ? ae(t, e) : null;
  }
  getValue() {
  ...
  onClose() {
    this.calciteComboboxClose.emit(), oe(this);
  }
  ...
  async refreshSelectionDisplay() {
    if (await Ie(this), x(this.selectionMode) || !this.textInput.value)
      return;
    const { allSelectedIndicatorChipEl: e, chipContainerEl: t, selectionDisplay: i, placeholder: s, selectedIndicatorChipEl: l, textInput: { value: a } } = this, o = parseInt(getComputedStyle(t).gap.replace("px", "")), n = v(t), { fontSize: h, fontFamily: p } = getComputedStyle(a), u = (te(s, `${h} ${p}`) || parseInt(K)) + o, w = v(e), D = v(l), G = Math.max(w, D);
  ...
  renderAllSelectedIndicatorChip() {
    ...
    })} .label=${l} .scale=${t} title=${l ?? m} value ${b(s)}>${l}</calcite-chip>`;
  }
  ...
  renderSelectedIndicatorChip() {
    ...
    return d`<calcite-chip class=${r({
      chip: !0,
      [c.chipInvisible]: n
    })} .label=${h} .scale=${s} title=${h ?? m} value ${b(o)}>${h}</calcite-chip>`;
  }
  ...
  renderInput() {
    ...
    return d`<span class=${r({
      ...
      label: !0,
      "label--icon": !!n?.icon
    })}>${$(n)}</span>`) || ""}${f("input", d`<input aria-activedescendant=${this.activeDescendant ?? m} aria-controls=${`${y}${e}`} aria-errormessage=${P.validationMessage} aria-owns=${`${y}${e}`} aria-autocomplete=list .ariaExpanded=${a} aria-haspopup=listbox .ariaInvalid=${this.status === "invalid"} .ariaLabel=${ge(this)} class=${r({
      [c.input]: !0,
      "input--single": !0,
      [c.inputHidden]: h,
      "input--icon": this.showingInlineIcon && !!this.placeholderIcon
    })} data-test-id=input .disabled=${t} .id=${`${B}${e}`} @focus=${this.comboboxFocusHandler} @input=${this.inputHandler} placeholder=${i ?? m} .readOnly=${this.readOnly} role=combobox .tabIndex=${this.activeChipIndex === -1 ? 0 : -1} type=text .value=${N(this.filterText ?? "")} ${b(this.textInput)}>`)}</span>`;
  }
  ...
  renderFloatingUIContainer() {
    ...
    return d`<div aria-hidden=true class=${r(c.floatingUIContainer)} ${b(e)}><div class=${r(s)} ${b(t)}><ul class=${r({ list: !0, "list--hide": !i })}><slot></slot></ul></div></div>`;
  }
