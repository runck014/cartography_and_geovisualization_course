...
import { LitElement as p, safeClassMap as o, nothing as i } from "@arcgis/lumina";
import { c as v } from "../../chunks/loadable.js";
...
class x extends p {
  ...
  async setFocus() {
    await v(this), this.href && this.el.focus();
  }
  // #endregion
  // #region Lifecycle
  load() {
  ...
  renderHeaderContent() {
    const { heading: n, headingLevel: a, description: r } = this, c = n ? s(t.heading, f({ class: {
      [t.heading]: !0,
      [t.standalone]: !this.description
    }, level: a, children: n })) : null, l = r ? s(t.description, e`<span class=${o(t.description)}>${r}</span>`) : null;
    return c || l ? s(t.textContainer, e`<div class=${o(t.textContainer)}>${c}${l}</div>`) : null;
  }
  render() {
    const { icon: n, href: a, label: r, rel: c, target: l, thumbnail: d } = this, g = e`${d && e`<img alt=${(r || "") ?? i} class=${o(t.image)} src=${d ?? i}>` || ""}${n && this.renderIcon() || ""}${this.renderHeaderContent()}`;
    return a ? e`<a class=${o({
      [t.container]: !0,
      [t.containerLink]: !0
    })} href=${a ?? i} rel=${c ?? i} target=${l ?? i}>${g}</a>` : e`<div class=${o(t.container)}>${g}</div>`;
  }
}
h("calcite-navigation-logo", x);
export {
  x as NavigationLogo
};
