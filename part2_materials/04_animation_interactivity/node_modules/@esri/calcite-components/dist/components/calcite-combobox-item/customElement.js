...
import { html as o, nothing as y } from "lit";
import { LitElement as I, createEvent as m, safeClassMap as i, safeStyleMap as k } from "@arcgis/lumina";
...
import { w as D, g as b } from "../../chunks/component.js";
import { r as F } from "../../chunks/dom.js";
...
class U extends I {
  ...
  render() {
    const { disabled: e, heading: c, label: u, textLabel: g, value: f, filterTextMatchPattern: l, description: d, shortHeading: h } = this, a = T(this.selectionMode), x = e || a ? void 0 : "check", v = a ? "bullet-point" : "check", w = c || g, C = u || f, $ = {
      [t.label]: !0,
      [t.selected]: this.selected,
      [t.active]: this.active,
      [t.single]: a
    }, z = E(this.el);
    return this.el.ariaHidden = "true", this.el.ariaLabel = C, M({ disabled: e, children: o`<div class=${i({
      [t.container]: !0,
      [t.scale(this.scale)]: !0
    })} style=${k({ "--calcite-combobox-item-spacing-indent-multiplier": `${z}` })}><li class=${i($)} id=${this.guid ?? y} @click=${this.itemClickHandler}>${this.renderSelectIndicator(v)}<slot name=${p.contentStart}></slot>${this.renderIcon(x)}<div class=${i(t.centerContent)}><div class=${i(t.title)}>${s({
      text: w,
      pattern: l
