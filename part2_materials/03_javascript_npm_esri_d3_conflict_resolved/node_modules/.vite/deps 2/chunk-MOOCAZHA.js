...
async function v(n2, o, s2 = {}, a = 5) {
  ...
  const { apiKey: u2, customParameters: d, signal: p2 } = s2, y = _(c.href, n2.landingPage.url), h = { limit: a, ...d, token: u2 }, T2 = Et(y, h), k2 = { accept: S.geojson }, { data: x2 } = await U(T2, { signal: p2, headers: k2 }), v2 = A(T2, a, x2.links) ?? I3;
  E(x2);
  const N3 = I2(x2, { geometryType: o.geometryType }), O2 = o.fields || N3.fields || [], P2 = null != o.hasZ ? o.hasZ : N3.hasZ, q2 = N3.geometryType, C2 = o.objectIdField || N3.objectIdFieldName || "OBJECTID";
  let R2 = o.timeInfo;
  ...
  if (R2) {
    ...
    R2.timeReference || (R2.timeReference = { timeZoneIANA: i }), R2.startTimeField || R2.endTimeField || (F().warn({ name: "ogc-feature-layer:invalid-timeInfo", message: "startTimeField and endTimeField are missing", details: { timeInfo: R2 } }), R2 = void 0);
  }
  return { drawingInfo: q2 ? u(q2) : null, extent: K(n2), geometryType: q2, fields: O2, hasZ: !!P2, objectIdField: C2, paginationParameter: v2, timeInfo: R2 };
}
async function N2(n2, r = {}) {
  const { links: o, url: s2 } = n2, a = L(o, "data", S.json) || L(o, "http://www.opengis.net/def/rel/ogc/1.0/data", S.json);
  if (null == a) throw new s("ogc-feature-layer:missing-collections-page", "Missing collections url");
  const { apiKey: l, customParameters: c, signal: u2 } = r, d = _(a.href, s2), { data: f } = await U(d, { signal: u2, headers: { accept: S.json }, query: { ...c, token: l } });
  for (const e2 of f.collections) e2.landingPage = n2;
  return f;
}
async function O(n2, r = {}) {
  const { links: o, url: s2 } = n2, a = L(o, "conformance", S.json) || L(o, "http://www.opengis.net/def/rel/ogc/1.0/conformance", S.json);
  if (null == a) throw new s("ogc-feature-layer:missing-conformance-page", "Missing conformance url");
  const { apiKey: l, customParameters: c, signal: u2 } = r, d = _(a.href, s2), { data: f } = await U(d, { signal: u2, headers: { accept: S.json }, query: { ...c, token: l } });
  return f;
}
async function P(t, n2 = {}) {
  const { apiKey: i3, customParameters: r, signal: o } = n2, { data: s2 } = await U(t, { signal: o, headers: { accept: S.json }, query: { ...r, token: i3 } });
  return s2.url = t, s2;
}
async function q(t, n2 = {}) {
  const { links: r, url: o } = t, s2 = L(r, "service-desc", S.openapi);
  ...
  const { apiKey: a, customParameters: l, signal: c } = n2, u2 = _(s2.href, o), { data: d } = await U(u2, { signal: c, headers: { accept: S.openapi }, query: { ...l, token: a } });
  return d;
}
function C(e2) {
...
async function $(n2, r, o) {
  const { collection: { links: l, landingPage: { url: f } }, layerDefinition: m, maxRecordCount: g2, queryParameters: { apiKey: j, customParameters: b }, spatialReference: F2, supportedCrs: I4 } = n2, T2 = L(l, "items", S.geojson) || L(l, "http://www.opengis.net/def/rel/ogc/1.0/items", S.geojson);
  ...
  const P2 = g.fromJSON(F2), q2 = r.outSpatialReference ?? P2, C2 = q2.isWGS84 ? null : W(q2, I4), R2 = U3(k2, I4), $2 = M2(N3), G2 = Z2(O2), D2 = x2 ?? (null == v2 ? g2 : 10), K2 = 0 === v2 ? void 0 : v2, { fields: A2, geometryType: J, hasZ: z, objectIdField: E2, paginationParameter: _2 } = m, B = _(T2.href, f), { data: Q } = await U(B, { ...o, query: { ...b, ...R2, crs: C2, datetime: $2, query: G2, limit: D2, [_2]: K2, token: j }, headers: { accept: S.geojson } }), V = N(Q, { geometryType: J, hasZ: z, objectIdField: E2 }), H = V.length === D2 && !!L(Q.links ?? [], "next", S.geojson), X = new Z(A2);
  for (const e2 of V) {
    const t = {};
    p(X, t, e2.attributes, true);
  ...
  if (!C2 && q2.isWebMercator) {
    for (const e2 of V) if (null != e2.geometry && null != J) {
      ...
      t.spatialReference = g.WGS84, e2.geometry = ot(M(t, q2));
    }
  }
  for (const e2 of V) e2.objectId = e2.attributes[E2];
  const Y = C2 || !C2 && q2.isWebMercator ? q2.toJSON() : U2, ee = new e();
...
function U3(e2, t) {
  ...
  const i3 = W(n2, t);
  return null != i3 ? { bbox: D(e2), "bbox-crs": i3 } : n2.isWebMercator ? { bbox: D(M(e2, g.WGS84)) } : null;
}
function K(e2) {
  var _a;
  ...
  return { xmin: r, ymin: o, xmax: i3 ? n2[2] : n2[3], ymax: i3 ? n2[3] : n2[4], zmin: s2, zmax: i3 ? void 0 : n2[5], spatialReference: g.WGS84.toJSON() };
}
function L(e2, t, n2) {
...
//# sourceMappingURL=chunk-MOOCAZHA.js.map
