...
function g(e, r2, c2, i2, u2, a2, f = null) {
  const s2 = d(e, r2);
  if (!s2) return;
  const m2 = s2.extent, { cache: x2, rasterInfo: h2 } = s2;
  ...
  for (let d2 = 0; d2 < g2.length; d2++) {
    ...
    const c3 = new j({ x: i2, y: i2, spatialReference: e2.spatialReference });
    if (null == u2 && !(u2 = C(c3, y, e2, f))) return;
    const { pyramidLevel: s3, pyramidResolution: m3, excessiveReading: x3 } = le(u2, h2, a2 || "closest");
    if (x3) return;
    const { storageInfo: M } = h2, { origin: R } = M, C2 = { x: Math.max(0, Math.floor((r3.xmin - R.x) / m3.x)), y: Math.max(0, Math.floor((R.y - r3.ymax) / m3.y)) }, B = Math.ceil((r3.xmax - r3.xmin) / m3.x - 0.1), j2 = Math.ceil((r3.ymax - r3.ymin) / m3.y - 0.1), v = s3 > 0 ? M.pyramidBlockWidth : M.blockWidth, b = s3 > 0 ? M.pyramidBlockHeight : M.blockHeight, w = 1, $ = Math.max(0, Math.floor(C2.x / v) - w), I = Math.max(0, Math.floor(C2.y / b) - w), H = Math.floor((C2.x + B - 1) / v) + w, E = Math.floor((C2.y + j2 - 1) / b) + w;
...
//# sourceMappingURL=chunk-SHWUES4N.js.map
