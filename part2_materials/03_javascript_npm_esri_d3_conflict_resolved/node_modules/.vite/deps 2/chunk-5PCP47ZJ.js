...
var w;
var x2 = new n({ PNG: "png", PNG8: "png8", PNG24: "png24", PNG32: "png32", JPEG: "jpg", JPG: "jpg", DIB: "dib", TIFF: "tiff", EMF: "emf", PS: "ps", PDF: "pdf", GIF: "gif", SVG: "svg", SVGZ: "svgz", Mixed: "mixed", MIXED: "mixed", LERC: "lerc", LERC2D: "lerc2d", RAW: "raw", pbf: "pbf" });
var j2 = w = class extends f {
  static create(e = {}) {
    const { resolutionFactor: t2 = 1, scales: o2, size: r3 = 256, spatialReference: i2 = g2.WebMercator, numLODs: l2 = 24 } = e;
    if (!g(i2)) {
      const e2 = [];
      if (o2) for (let t3 = 0; t3 < o2.length; t3++) {
        ...
        let t3 = 5e-4;
        for (let o3 = l2 - 1; o3 >= 0; o3--) e2.unshift(new p({ level: o3, scale: t3, resolution: t3 })), t3 *= 2;
      }
      return new w({ dpi: 96, lods: e2, origin: new j(0, 0, i2), size: [r3, r3], spatialReference: i2 });
    }
    const n2 = d(i2), p2 = e.origin ? new j({ x: e.origin.x, y: e.origin.y, spatialReference: i2 }) : new j(n2 ? { x: n2.origin[0], y: n2.origin[1], spatialReference: i2 } : { x: 0, y: 0, spatialReference: i2 }), u2 = 96, d2 = 1 / (z(i2) * 39.37 * u2), g3 = [];
    if (o2) for (let s2 = 0; s2 < o2.length; s2++) {
      const e2 = o2[s2], t3 = e2 * d2;
      g3.push(new p({ level: s2, scale: e2, resolution: t3 }));
    }
    else {
      let e2 = P(i2) ? 512 / r3 * 5916575275917094e-7 : 256 / r3 * 591657527591555e-6;
      const o3 = Math.ceil(l2 / t2);
      g3.push(new p({ level: 0, scale: e2, resolution: e2 * d2 }));
      for (let s2 = 1; s2 < o3; s2++) {
        const o4 = e2 / 2 ** t2, r4 = o4 * d2;
        g3.push(new p({ level: s2, scale: o4, resolution: r4 })), e2 = o4;
      }
    }
    return new w({ dpi: u2, lods: g3, origin: p2, size: [r3, r3], spatialReference: i2 });
  }
  constructor(e) {
  ...
  get isWrappable() {
    ...
    if (e && t2) {
      const o2 = d(e);
      return e.isWrappable && !!o2 && Math.abs(o2.origin[0] - t2.x) <= o2.dx;
    }
    return false;
  }
  readOrigin(e, t2) {
  ...
  set lods(e) {
    ...
    e && (t2 = -1 / 0, o2 = 1 / 0, e.forEach((e2) => {
      s2.push(e2.scale), t2 = e2.scale > t2 ? e2.scale : t2, o2 = e2.scale < o2 ? e2.scale : o2, r3[e2.level] = e2;
  ...
  zoomToScale(e) {
    ...
    const o2 = Math.floor(e), s2 = o2 + 1;
    return t2[o2] / (t2[o2] / t2[s2]) ** (e - o2);
  }
  scaleToZoom(e) {
    const t2 = this.scales, o2 = t2.length - 1;
    let s2 = 0;
    for (; s2 < o2; s2++) {
      const o3 = t2[s2], r3 = t2[s2 + 1];
      if (o3 <= e) return s2;
      if (r3 === e) return s2 + 1;
      if (o3 > e && r3 < e) return s2 + Math.log(o3 / e) / Math.log(o3 / r3);
    }
    return s2;
  }
  tileAt(e, t2, o2, s2) {
    ...
    else if (s(t2.spatialReference, this.spatialReference)) i2 = t2.x, l2 = t2.y, s2 = o2;
    else {
      const e2 = M(t2, this.spatialReference);
      if (null == e2) return null;
      i2 = e2.x, l2 = e2.y, s2 = o2;
    }
    const n2 = r3.resolution * this.size[0], p2 = r3.resolution * this.size[1];
    return s2 || (s2 = new t(null, 0, 0, 0, u())), s2.level = e, s2.row = Math.floor((this.origin.y - l2) / p2 + 1e-3), s2.col = Math.floor((i2 - this.origin.x) / n2 + 1e-3), this.updateTileInfo(s2), s2;
  }
  updateTileInfo(e, t2 = w.ExtrapolateOptions.NONE) {
    let o2 = this.lodAt(e.level);
    if (!o2 && t2 === w.ExtrapolateOptions.POWER_OF_TWO) {
      const t3 = this.lods[this.lods.length - 1];
      t3.level < e.level && (o2 = t3);
    }
    if (!o2) return;
    const s2 = e.level - o2.level, r3 = o2.resolution * this.size[0] / 2 ** s2, i2 = o2.resolution * this.size[1] / 2 ** s2;
  ...
  getTileBounds(e, t2) {
    ...
    const { resolution: s2 } = o2, r3 = s2 * this.size[0], i2 = s2 * this.size[1];
  ...
  getCompatibleForVTL(e) {
    ...
    for (let r3 = 0; r3 < s2; r3++) {
      ...
      o2.push(new p({ level: r3, scale: s3, resolution: i2 }));
    }
    return new w({ size: [e, e], dpi: this.dpi, format: this.format, compressionQuality: this.compressionQuality, origin: this.origin, spatialReference: this.spatialReference, lods: o2 });
  }
...
r([m({ type: Number, json: { write: true } })], j2.prototype, "compressionQuality", void 0), r([m({ type: Number, json: { write: true } })], j2.prototype, "dpi", void 0), r([m({ type: String, json: { read: x2.read, write: x2.write, origins: { "web-scene": { read: false, write: false } } } })], j2.prototype, "format", void 0), r([m({ readOnly: true })], j2.prototype, "isWrappable", null), r([m({ type: j, json: { write: true } })], j2.prototype, "origin", void 0), r([o("origin")], j2.prototype, "readOrigin", null), r([m({ type: [p], value: null, json: { write: true } })], j2.prototype, "lods", null), r([m({ readOnly: true })], j2.prototype, "scales", void 0), r([m({ cast: (e) => Array.isArray(e) ? e : "number" == typeof e ? [e, e] : [256, 256] })], j2.prototype, "size", void 0), r([o("size", ["rows", "cols"])], j2.prototype, "readSize", null), r([r2("size", { cols: { type: x }, rows: { type: x } })], j2.prototype, "writeSize", null), r([m({ type: g2, json: { write: true } })], j2.prototype, "spatialReference", void 0), j2 = w = r([a("esri.layers.support.TileInfo")], j2), function(e) {
  var t2;
...
//# sourceMappingURL=chunk-5PCP47ZJ.js.map
