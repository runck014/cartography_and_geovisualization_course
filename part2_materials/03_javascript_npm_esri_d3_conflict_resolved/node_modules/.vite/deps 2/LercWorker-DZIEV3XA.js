...
var t = {};
t.defaultNoDataValue = w(-1 / 0), t.decode = function(s2, r2) {
  var o2 = (r2 = r2 || {}).encodedMaskData || null === r2.encodedMaskData, f2 = l(s2, r2.inputOffset || 0, o2), u = null != r2.noDataValue ? w(r2.noDataValue) : t.defaultNoDataValue, m = i(f2, r2.pixelType || Float32Array, r2.encodedMaskData, u, r2.returnMask), d = { width: f2.width, height: f2.height, pixelData: m.resultPixels, minValue: f2.pixels.minValue, maxValue: f2.pixels.maxValue, noDataValue: u };
...
var i = function(e, t2, i2, n2, a2) {
  ...
  for (var g, h, k = new Float32Array(m * d), x = 0; x <= u; x++) {
    ...
    if (0 !== p) for (var w2 = 0; w2 <= f2; w2++) {
      var y = w2 !== f2 ? m : e.width % f2;
      if (0 !== y) {
        var V, B, v, U, D = x * e.width * d + w2 * m, M = e.width - y, b = e.pixels.blocks[o2];
...
var l = function(e, t2, i2) {
  ...
  for (var h = 1e9, k = 0, x = 0; x < g; x++) for (var p = 0; p < c; p++) {
    var w2 = 0, y = e.byteLength - t2;
    l2 = new DataView(e, t2, Math.min(10, y));
    var V = {};
    n2.pixels.blocks[k++] = V;
    var B = l2.getUint8(0);
    if (w2++, V.encoding = 63 & B, V.encoding > 3) throw "Invalid block encoding (" + V.encoding + ")";
    if (2 !== V.encoding) {
      if (0 !== B && 2 !== B) {
        if (B >>= 6, V.offsetType = B, 2 === B) V.offset = l2.getInt8(1), w2++;
        else if (1 === B) V.offset = l2.getInt16(1, true), w2 += 2;
        else {
          if (0 !== B) throw "Invalid block offset type";
          V.offset = l2.getFloat32(1, true), w2 += 4;
        }
        if (h = Math.min(V.offset, h), 1 === V.encoding) if (B = l2.getUint8(w2), w2++, V.bitsPerPixel = 63 & B, B >>= 6, V.numValidPixelsType = B, 2 === B) V.numValidPixels = l2.getUint8(w2), w2++;
        else if (1 === B) V.numValidPixels = l2.getUint16(w2, true), w2 += 2;
        else {
          if (0 !== B) throw "Invalid valid pixel count type";
          V.numValidPixels = l2.getUint32(w2, true), w2 += 4;
        }
      }
      var v;
      if (t2 += w2, 3 != V.encoding) {
        if (0 === V.encoding) {
...
//# sourceMappingURL=LercWorker-DZIEV3XA.js.map
