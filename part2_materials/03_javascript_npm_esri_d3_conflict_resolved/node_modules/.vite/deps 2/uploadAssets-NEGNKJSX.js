...
async function m4({ data: m5, name: f2, description: d2 }, u3, h2) {
  let w = null;
  try {
    const y = V(u3, "uploads"), j = V(y, "info"), { data: g4 } = await U(j, { query: { f: "json" }, responseType: "json" });
    s(h2);
    const q2 = g2(u3), z2 = g4.maxUploadFileSize * n6, T = q2 ? p3 : z2, U2 = q2 ? Math.min(i4, z2) : i4;
    if (m5.size > T) throw new Error("Data too large");
    const A2 = V(y, "register"), { data: E } = await U(A2, { query: { f: "json", itemName: c3(f2), description: d2 }, responseType: "json", method: "post" });
    if (s(h2), !E.success) throw new Error("Registration failed");
    const { itemID: P } = E.item;
    w = V(y, P);
    const D = V(w, "uploadPart"), I = Math.ceil(m5.size / U2), M2 = new Array();
    for (let e3 = 0; e3 < I; ++e3) M2.push(m5.slice(e3 * U2, Math.min((e3 + 1) * U2, m5.size)));
    const v = M2.slice().reverse(), x = new Array(), F = i3(I, h2 == null ? void 0 : h2.onProgress, "uploadItem"), _2 = async () => {
      for (; 0 !== v.length; ) {
        const t4 = M2.length - v.length, r5 = v.pop(), s3 = new FormData(), n7 = F.simulate(t4, h(r5.size));
        try {
          s3.append("f", "json"), s3.append("file", r5), s3.append("partId", `${t4}`);
          const { data: a6 } = await U(D, { timeout: 0, body: s3, responseType: "json", method: "post" });
    ...
    for (let e3 = 0; e3 < l4 && 0 !== v.length; ++e3) x.push(_2());
    await Promise.all(x);
    const b2 = V(w, "commit"), { data: C } = await U(b2, { query: { f: "json", parts: M2.map((e3, o) => o).join(",") }, responseType: "json", method: "post" });
    if (s(h2), !C.success) throw new Error("Commit failed");
    return C.item;
  } catch (y) {
    if (null != w) {
      const o = V(w, "delete");
      await U(o, { query: { f: "json" }, responseType: "json", method: "post" });
    }
    throw y;
  }
}
...
function W(e3, { infoFor3D: s3 }, t4 = {}) {
  if (!e3) return null;
  const { supportedFormats: r5, editFormats: o } = s3, n7 = N(e3), a6 = new Array(), i5 = m(s3), c4 = l(s3);
  let u3 = false;
  for (const l5 of n7) {
    const e4 = K(l5, r5);
    if (!e4) return null;
...
async function Y(e3, s3, r5) {
  ...
  try {
    ...
    a(l5), ie().warnOnce(`Service ${s3.url} does not support the REST Uploads API.`);
  }
  if (!u3) {
    const e4 = await Wt(n7);
...
async function _(s3, r5, o) {
  ...
  try {
    const n8 = await U(V(r5.parsedUrl.path, "uploadAssets"), { timeout: 0, query: { f: "json", assets: JSON.stringify(s3) }, method: "post", responseType: "json" });
...
function te(e3, s3) {
  ...
  if (has("enable-feature:georeferenced-uploads") && s3.transform) {
    if (t4.transform = i2(s3.transform), s3.spatialReference) {
      const e4 = g.fromJSON(s3.spatialReference);
...
async function oe(s3, t4) {
  const o = (await U(s3, t4)).data.statusUrl;
  for (; ; ) {
    const s4 = (await U(o, { query: { f: "json" }, responseType: "json" })).data;
    switch (s4.status) {
      case "Completed":
        return U(s4.resultUrl, { query: { f: "json" }, responseType: "json" });
...
//# sourceMappingURL=uploadAssets-NEGNKJSX.js.map
