...
var m2 = class {
  constructor(t, e, i3) {
    ...
    const { buffer: s3, pixelType: r2, textureOnly: a } = t, n4 = d(r2);
    this.blockIndex = i3, this.pixelType = r2, this.size = e, this.textureOnly = a, a || (this.data = new n4(s3)), this._resetRange();
  }
  destroy() {
    var _a;
  ...
  setData(t, e, i3) {
    ...
    !r2 || a >= r2.length || (r2[a] = i3, this.dirtyStart = Math.min(this.dirtyStart, s3), this.dirtyEnd = Math.max(this.dirtyEnd, s3));
  }
  getData(t, e) {
    if (null == this.data) return null;
    const i3 = f(t) * this.texelSize + e;
  ...
  getFBO(t, e = 0) {
    if (!this._fbos[e]) {
      const i3 = 0 === e ? this.getTexture(t) : this._textureDesc;
      this._fbos[e] = new x2(t, i3);
    }
    return this._fbos[e];
  }
  get hasDirty() {
  ...
  updateTexture(e, i3) {
    try {
      ...
      has("esri-2d-update-debug") && console.debug(`Version[${i3}] AttributeStoreView.updateTexture`, { start: s3, end: r2, firstBytes: new Uint8Array(this.data.buffer.slice(0, 16)), block: this }), this._resetRange();
      const a = this.data.buffer, n4 = this.getTexture(e), u = 4, h = (s3 - s3 % this.size) / this.size, o = (r2 - r2 % this.size) / this.size, l = h, d2 = this.size, _ = o, p2 = h * this.size * u, c2 = (d2 + _ * this.size) * u - p2, x3 = d(this.pixelType), f2 = new x3(a, p2 * x3.BYTES_PER_ELEMENT, c2), b2 = this.size, m3 = _ - l + 1;
      if (m3 > this.size) return void T().error(new s("mapview-webgl", "Out-of-bounds index when updating AttributeData"));
      n4.updateData(0, 0, l, b2, m3, f2);
    } catch (s3) {
    }
  }
  update(t) {
    const { data: e, start: i3, end: s3 } = t;
    if (null != e && null != this.data) {
      const s4 = this.data, r2 = i3 * this.texelSize;
      for (let i4 = 0; i4 < e.length; i4++) {
        const a = 1 << i4 % this.texelSize;
        t.layout & a && (s4[r2 + i4] = e[i4]);
      }
    }
    this.dirtyStart = Math.min(this.dirtyStart, i3), this.dirtyEnd = Math.max(this.dirtyEnd, s3);
  }
  resize(t, e) {
    const i3 = this.size;
    if (this.size = e, this.textureOnly) return void (i3 !== this.size && (this._lastTexture = this._texture, this._texture = null));
    const s3 = d(this.pixelType);
  ...
  _initTexture(t) {
    const e = new m(t, this._textureDesc, this.data ?? void 0);
    if (null != this._lastTexture && this._fbos[0]) {
      const i3 = this._lastTexture.descriptor.width, s3 = this._lastTexture.descriptor.height, r2 = this._lastTexture.descriptor.dataType, a = this._lastTexture.descriptor.pixelFormat, n4 = this.getFBO(t), u = c(r2), h = new (d(r2))(new ArrayBuffer(i3 * s3 * u * this.texelSize)), o = t.getBoundFramebufferObject(), { x: l, y: d2, width: p2, height: c2 } = t.getViewport();
      t.bindFramebuffer(n4), n4.readPixels(0, 0, i3, s3, a, r2, h), e.updateData(0, 0, 0, 2 * i3, s3 / 2, h), t.setViewport(l, d2, p2, c2), t.bindFramebuffer(o);
    }
...
var y2 = class {
  ...
  _initialize(t) {
    ...
    if (this.size = t.blockSize, has("esri-2d-update-debug") && console.debug("AttributeStoreView.initialize", { message: t }), null == this._data) this._data = e.map((t2, e2) => null != t2 ? new m2(t2, this.size, e2) : null);
    else for (let i3 = 0; i3 < this._data.length; i3++) {
      const t2 = this._data[i3], s3 = e[i3];
      null != s3 && (null == t2 ? this._data[i3] = new m2(s3, this.size, i3) : t2.resize(s3, this.size));
    }
  }
  destroy() {
    var _a;
  ...
  getData(t, e, i3, s3) {
    ...
    if (null == r2) return 0;
    const a = r2.getData(t, i3);
    return null != a ? a : s3;
  }
  setData(t, e, i3, s3) {
    this._data[e].setData(t, i3, s3);
  }
  lockTextureUploads() {
    this._locked = true;
  }
  ...
  update() {
    ...
    for (const e of t) {
      const { blockData: t2, initArgs: i3, sendUpdateEpoch: s3, version: r2 } = e;
      has("esri-2d-update-debug") && console.debug(`Version[${this._version}] Epoch[${s3}] AttributeStoreView.applyUpdate`), this._version = r2, this._epoch = s3, null != i3 && this._initialize(i3);
      const a = this._data;
      for (let e2 = 0; e2 < t2.length; e2++) {
        const i4 = t2[e2], s4 = a[e2];
        null != s4 && null != i4 && (has("esri-2d-update-debug") && console.debug(`Version[${this._version}] CpuBlock[${e2}] AttributeStoreView.update`, { block: i4 }), s4.update(i4));
      }
    }
  }
  getUniforms(t) {
  ...
  _getTexture(t, e) {
    var _a;
    const i3 = (_a = this._data) == null ? void 0 : _a[e];
  ...
  _getDefaultTexture(t) {
    if (null == this._defaultTexture) {
      const e = new p();
      e.wrapMode = D.CLAMP_TO_EDGE, e.samplingMode = L.NEAREST, e.width = 1, e.height = 1, this._defaultTexture = new m(t, e, new Uint8Array(4));
    }
...
var n2 = class extends i {
  ...
  restartAnimation(t) {
    ...
    if (!e) return;
    const i3 = performance.now() / 1e3;
    e.firstIndexed = i3, e.dirty = true;
  }
  restartAllAnimations() {
    const t = performance.now() / 1e3;
    for (const [e, i3] of this._entityIndex) i3.firstIndexed = t, i3.dirty = true;
  }
  _reindexFeatures() {
    const t = performance.now() / 1e3;
    for (const e of this.children) for (const i3 of e.entityIds) {
      const { objectId: e2 } = i3;
      let s3 = this._entityIndex.get(e2);
      s3 || (s3 = { objectId: e2, displayId: 0, firstIndexed: t, lastIndexed: 0, dirty: true }, this._entityIndex.set(e2, s3)), s3.lastIndexed = t, s3.displayId = i3.displayId;
    }
    for (const [e, i3] of this._entityIndex) t - i3.lastIndexed > s2 && this._entityIndex.delete(e);
  }
...
var i2 = class {
  constructor(t, i3, e) {
    this._instanceId = t, this.techniqueRef = i3, this._input = e;
  }
  get instanceId() {
    return n3(this._instanceId);
  }
  createMeshInfo(i3) {
    return { id: n3(this._instanceId), techniqueType: this.techniqueRef.type, inputParams: i3, optionalAttributes: this._input.optionalAttributes };
  }
  getInput() {
    return this._input;
  }
...
//# sourceMappingURL=chunk-5ZJ2MCRU.js.map
