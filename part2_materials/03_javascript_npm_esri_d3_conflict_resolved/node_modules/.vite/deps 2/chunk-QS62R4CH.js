...
function s3(s4) {
  const a4 = s4 == null ? void 0 : s4.match(t);
  if (!a4) return null;
  const [, r2, c2, o] = a4;
  if (!r2) return null;
  let l2 = null, m2 = null, n3 = null;
  const { devext: u, qaext: i, www: p2 } = e;
  if (c2) if (l2 = r2, o) switch (o.toLowerCase()) {
    case "devext":
      ({ customBaseUrl: m2, portalHostname: n3 } = u);
      break;
    case "qa":
  ...
  return { customBaseUrl: m2, isPortal: false, portalHostname: n3, urlKey: l2 };
}
function a2(t3) {
  return /\/(sharing|usrsvcs)\/(appservices|servers)\//i.test(t3);
}
function r(t3) {
  const e2 = /^https?:\/\/(?:cdn|[a-z\d-]+\.maps)\.arcgis\.com/i, s4 = /^https?:\/\/(?:cdndev|[a-z\d-]+\.mapsdevext)\.arcgis\.com/i, a4 = /^https?:\/\/(?:cdnqa|[a-z\d-]+\.mapsqa)\.arcgis\.com/i;
  return e2.test(t3) ? t3 = t3.replace(e2, "https://www.arcgis.com") : s4.test(t3) ? t3 = t3.replace(s4, "https://devext.arcgis.com") : a4.test(t3) && (t3 = t3.replace(a4, "https://qaext.arcgis.com")), t3;
}
...
function v(t3, n3) {
  return t3 ? n3 && "function" == typeof n3 ? Object.keys(t3).map((e2) => encodeURIComponent(e2) + "=" + encodeURIComponent(n3(e2, t3[e2]))).join("&") : Object.keys(t3).map((e2) => {
    ...
    return s4 ? i + encodeURIComponent(s4(o)) : Array.isArray(o) ? o.map((t4) => a(t4) ? i + encodeURIComponent(JSON.stringify(t4)) : i + encodeURIComponent(t4)).join("&") : a(o) ? i + encodeURIComponent(JSON.stringify(o)) : i + encodeURIComponent(o);
  }).filter((t4) => t4).join("&") : "";
}
function P(t3 = false) {
...
function W(t3, n3) {
  ...
  return null != e2 && null != r2 ? e2.portalHostname === r2.portalHostname : null == e2 && null == r2 && F2(t3, n3, true);
}
function N(t3, n3) {
  return t3 = z(t3), n3 = z(n3), Ot(t3) === Ot(n3);
}
function z(t3) {
...
function M(t3) {
  ...
  if (F2(t3, U)) return true;
  const n3 = c.trustedServers || [];
  for (let e2 = 0; e2 < n3.length; e2++) {
    const r2 = Q(n3[e2]);
    for (let n4 = 0; n4 < r2.length; n4++) if (F2(t3, r2[n4])) return true;
  }
  return false;
}
function Q(t3) {
...
function G(t3, n3 = q, e2) {
  ...
  const u = (t4, n4, e3) => -1 === (e3 = t4.indexOf(n4, e3)) ? t4.length : e3;
  let l2 = u(o, "/", o.indexOf("//") + 2), c2 = -1;
  for (; o.slice(0, l2 + 1) === i.slice(0, l2) + "/" && (c2 = l2 + 1, l2 !== o.length); ) l2 = u(o, "/", l2 + 1);
  if (-1 === c2) return t3;
  if (s4 && c2 < s4.length) return t3;
  t3 = r2.slice(c2);
  const f2 = i.slice(c2 - 1).replaceAll(/[^/]+/g, "").length;
  if (f2 > 0) for (let a4 = 0; a4 < f2; a4++) t3 = `../${t3}`;
  else t3 = `./${t3}`;
  return t3;
}
function K(t3) {
  return t3 = It(t3 = Lt(t3 = bt(t3 = _(t3 = t3.trim()))));
}
function V(...t3) {
  const e2 = t3.filter(F);
...
function Ot(t3, n3 = false) {
  return pt(t3) ? t3.slice(2) : (t3 = t3.replace(a3, ""), n3 && t3.length > 1 && "/" === t3[0] && "/" === t3[1] && (t3 = t3.slice(2)), t3);
}
function Ut(t3) {
...
function It(t3) {
  ...
  return xt() && r2 === U.authority && !y.test(t3) || (wt() && r2 === U.authority || n3 && n3.some((t4) => r2 === t4 || r2.endsWith(`.${t4}`)) || wt() && !T(t3)) && (t3 = $t(t3)), t3;
}
...
//# sourceMappingURL=chunk-QS62R4CH.js.map
