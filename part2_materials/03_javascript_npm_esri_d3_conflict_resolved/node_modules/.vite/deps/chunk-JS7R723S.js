...
function c(e, t = false) {
  return { json: { read: { source: e, reader: (t2, n) => n[e] ? f(n[e]) : null }, write: !!t && { allowNull: true, writer(t2, n) {
    n[e] = t2 ? l(t2) : null;
  } } } };
}
function f(e, t = o) {
...
function l(e) {
  return { timeZoneIANA: e };
}
function Z(t, n = o) {
  if (!t || !a.has(t.timeZone)) return n;
  const r2 = a.get(t.timeZone);
  return m(t.timeZone) || t.respectsDaylightSaving ? r2 : w(r2);
}
function w(e) {
  const o2 = DateTime.local().setZone(e), r2 = Math.min(o2.set({ month: 1, day: 1 }).offset, o2.set({ month: 5 }).offset);
  if (0 === r2) return "Etc/UTC";
  return `Etc/GMT${FixedOffsetZone.instance(-r2).formatOffset(0, "narrow")}`;
}
function p(e, t, n, o2, s2) {
  if (t && "date" === s2) return { timeZone: i, timeZoneName: void 0 };
  const u2 = o2.timeStyle || o2.hour, m2 = n === r;
  return { timeZone: m2 ? "timestamp-offset" === s2 ? void 0 : e ?? i : n, timeZoneName: m2 && u2 ? a2 : void 0 };
}
function h(e) {
  const t = /* @__PURE__ */ new Set(["etc/utc", "etc/gmt", "gmt"]), n = new Set(Intl.supportedValuesOf("timeZone").map((e2) => e2.toLowerCase()));
...
//# sourceMappingURL=chunk-JS7R723S.js.map
