import {
  F
} from "./chunk-NHJGPXZH.js";
import {
  o
} from "./chunk-JS7R723S.js";
import {
  DateTime,
  IANAZone
} from "./chunk-Q7TEBZSN.js";

// node_modules/@arcgis/core/core/timeUtils.js
var r = { milliseconds: 1, seconds: 1e3, minutes: 6e4, hours: 36e5, days: 864e5, weeks: 6048e5, months: 26784e5, years: 31536e6, decades: 31536e7, centuries: 31536e8 };
function o2(r2, o3, i2, c2 = o) {
  const u = new IANAZone(F(c2)), a = DateTime.fromJSDate(r2, { zone: u }), l = "decades" === i2 || "centuries" === i2 ? "year" : T(i2);
  return "decades" === i2 && (o3 *= 10), "centuries" === i2 && (o3 *= 100), a.plus({ [l]: o3 }).toJSDate();
}
function i(e, t, n = "milliseconds") {
  const s = e.getTime(), r2 = m(t, n, "milliseconds");
  return new Date(s + r2);
}
function c(r2, o3, i2 = o) {
  const c2 = new IANAZone(F(i2)), u = DateTime.fromJSDate(r2, { zone: c2 });
  if ("decades" === o3 || "centuries" === o3) {
    const e = u.startOf("year"), { year: t } = e, n = t - t % ("decades" === o3 ? 10 : 100);
    return e.set({ year: n }).toJSDate();
  }
  const a = T(o3);
  return u.startOf(a).toJSDate();
}
function m(e, t, n) {
  if (0 === e) return 0;
  return e * r[t] / r[n];
}
function T(e) {
  switch (e) {
    case "milliseconds":
      return "millisecond";
    case "seconds":
      return "second";
    case "minutes":
      return "minute";
    case "hours":
      return "hour";
    case "days":
      return "day";
    case "weeks":
      return "week";
    case "months":
      return "month";
    case "years":
      return "year";
  }
}

export {
  r,
  o2 as o,
  i,
  c,
  m
};
//# sourceMappingURL=chunk-MGMVRMXQ.js.map
