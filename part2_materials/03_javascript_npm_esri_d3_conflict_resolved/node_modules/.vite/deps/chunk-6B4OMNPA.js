import {
  ...
  r,
  s as s3
} from "./chunk-3GQ4PJDJ.js";
import {
  o as o2,
  r as r2
} from "./chunk-ECCHNTW6.js";
import {
  f as f2
...
async function i2(i3, n2, p2, m3) {
  const a = f2(i3), u = n2[0].spatialReference, c2 = { ...m3, responseType: "json", query: { ...a.query, f: "json", sr: c(u), target: JSON.stringify({ geometryType: p(n2[0]), geometries: n2 }), cutter: JSON.stringify(p2) } }, f3 = await U(a.path + "/cut", c2), { cutIndexes: y2, geometries: g2 = [] } = f3.data;
...
async function m(m3, p2, f3) {
  const n2 = "string" == typeof m3 ? I(m3) : m3, a = p2[0].spatialReference, u = p(p2[0]), l = { ...f3, query: { ...n2.query, f: "json", sr: c(a), geometries: JSON.stringify(r2(p2)) } }, { data: y2 } = await U(n2.path + "/simplify", l);
...
function M(t, n2) {
  if (!(t instanceof d2 || t instanceof v)) {
  ...
  for (const e of i3) {
    ...
    for (let o3 = 0; o3 < e.length - 1; o3++) {
      const s4 = e[o3][0], r3 = e[o3][1], i4 = e[o3 + 1][0], l2 = e[o3 + 1][1], c2 = Math.sqrt((i4 - s4) * (i4 - s4) + (l2 - r3) * (l2 - r3)), f3 = (l2 - r3) / c2, u = (i4 - s4) / c2, p2 = c2 / n2;
      if (p2 > 1) {
        for (let l3 = 1; l3 <= p2 - 1; l3++) {
          const e3 = l3 * n2, o5 = u * e3 + s4, i6 = f3 * e3 + r3;
          t2.push([o5, i6]);
        }
        const e2 = (c2 + Math.floor(p2 - 1) * n2) / 2, o4 = u * e2 + s4, i5 = f3 * e2 + r3;
        t2.push([o4, i5]);
      }
      t2.push([i4, l2]);
    }
  }
  return g(t) ? new v({ rings: l, spatialReference: t.spatialReference }) : new d2({ paths: l, spatialReference: t.spatialReference });
}
function w(t, e, n2) {
  if (e) {
    const e2 = M(t, 1e6);
...
async function R2(e, n2, r3) {
  ...
  const p2 = "string" == typeof n2 ? n2 : (n2 == null ? void 0 : n2.url) ?? s.geometryServiceUrl;
  let g2, y2, x2, d4, M2, v3, P2, L2, U2 = 0;
  const z = [], A = [];
  for (const t of e) if (null != t) if (g2 || (g2 = t.spatialReference, y2 = d(g2), x2 = g2.isWebMercator, v3 = x2 ? 102100 : 4326, d4 = r[v3].maxX, M2 = r[v3].minX, P2 = r[v3].plus180Line, L2 = r[v3].minus180Line), y2) if ("mesh" === t.type) A.push(t);
    ...
    A.push(e2.rings ? new v(e2) : e2);
  } else if (t.extent) {
    const e2 = t.extent, n3 = i(e2.xmin, M2) * (2 * d4);
    let o3 = 0 === n3 ? t.clone() : s3(t.clone(), n3);
    e2.offset(n3, 0);
    let { xmin: s4, xmax: r4 } = e2;
    s4 = Number(s4.toFixed(9)), r4 = Number(r4.toFixed(9)), e2.intersects(P2) && r4 !== d4 ? (U2 = r4 > U2 ? r4 : U2, o3 = w(o3, x2), z.push(o3), A.push("cut")) : e2.intersects(L2) && s4 !== M2 ? (U2 = r4 * (2 * d4) > U2 ? r4 * (2 * d4) : U2, o3 = w(o3, x2, 360), z.push(o3), A.push("cut")) : A.push(o3);
  } else A.push(t.clone());
  else A.push(t);
  else A.push(t);
  let F = i(U2, d4), N = -90;
  ...
  if (z.length > 0 && S > 0) {
    const t = j2(z, await i2(p2, z, k, r3)), n3 = [], o3 = [];
    for (let r4 = 0; r4 < A.length; r4++) {
      const s5 = A[r4];
      if ("cut" !== s5) o3.push(s5);
      else {
        const s6 = t.shift(), i4 = e[r4];
        null != i4 && "polygon" === i4.type && i4.rings && i4.rings.length > 1 && s6.rings.length >= i4.rings.length ? (n3.push(s6), o3.push("simplify")) : o3.push(x2 ? R(s6) : s6);
      }
    }
    if (!n3.length) return o3;
    const s4 = await m(p2, n3, r3), i3 = [];
    for (let e2 = 0; e2 < o3.length; e2++) {
      const t2 = o3[e2];
      "simplify" !== t2 ? i3.push(t2) : i3.push(x2 ? R(s4.shift()) : s4.shift());
    }
    return i3;
  }
  const C = [];
...
function v2(t) {
  ...
  return p2.xmin = c2, p2.xmax = u, p2.width < i3 ? (p2.xmin -= s4, p2.xmax -= s4, p2) : e;
}
function P(t, e, n2) {
  const o3 = d(n2);
  if (null == o3) return t;
  ...
  let u = t + (l - c2) * i3;
  const p2 = u - e;
  return p2 > r3 ? u -= i3 : p2 < s4 && (u += i3), u;
}
function L(t, e) {
  const n2 = d(e);
  if (n2) {
...
//# sourceMappingURL=chunk-6B4OMNPA.js.map
